...
### 0.84 — Codex quota 窗口纠正 + Reset 日期时间展示(2026-07-31)
- **按真实窗口解析**:`rate_limits.primary/secondary` 不再硬编码成 5h/周语义,改按 `window_minutes` 排序并生成标签；额度事件即使没有 token 增量也会被摄取。
- **通用额度优先**:读取 `limit_id`,优先保留通用 `codex` quota,避免被时间更晚的模型专属额度覆盖；滚动更新只返回单个窗口时合并仍未过期的另一个窗口,并为旧账本补一次迁移重选。
- **Reset 正式展示**:Today 面板新增 `↻ Reset` 数据行,按本机时区显示绝对日期时间；5h/周双窗口在同一字段内分行右对齐,悬停保留相对倒计时,同步增加透明窗口预留高度。
- README 中英文改为“可用限额窗口”,不再承诺所有计划固定同时提供 5h/周窗口。
- 验证:typecheck + 81 单测 + build + `git diff --check` 全绿。
